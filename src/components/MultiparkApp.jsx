...
import { ArrowLeft, LogOut, Mail, Phone, HelpCircle, ExternalLink, Star, Percent, ChevronLeft } from 'lucide-react';
...
const MultiparkApp = () => {
  ...
  const goBack = () => {
    if (navigationHistory.length > 0) {
      ...
      // Limpar estados específicos ao voltar
  ...
  const Notification = () => {
    ...
    return (
      <div className={`fixed top-4 right-4 p-4 rounded-lg border ${bgColor} ${textColor} shadow-lg z-50 animate-fade-in`}>
  ...
  const Breadcrumbs = () => {
    ...
    } else if (currentPage === 'city') {
      items = [
        { label: 'Dashboard', active: false, onClick: () => navigateTo('dashboard') },
        { label: selectedCity, active: true }
      ];
    }
    ...
    const handleLogin = () => {
      ...
      if (email && password) {
        ...
        if (partnerKey) {
          ...
          showNotification('Email não autorizado. Contacte o administrador para obter acesso.', 'error');
        }
      }
    };
        ...
        <div className="min-h-screen bg-gray-100 flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="space-y-6">
              ...
              {email && !emailToPartner[email.toLowerCase()] && email.includes('@') && !emailError && (
    ...
    const copyToClipboard = (brand) => {
      ...
      try {
        navigator.clipboard.writeText(url).then(() => {
          showNotification(`Link copiado para clipboard: ${url}`);
        }).catch(err => {
          console.error('Erro ao copiar: ', err);
          showNotification('Não foi possível copiar o link. Tente novamente.', 'error');
        });
      } catch (err) {
        console.error('Erro ao copiar: ', err);
        showNotification('Não foi possível copiar o link. Tente novamente.', 'error');
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          <button
                            ...
                            aria-label={`Abrir link para ${service}`}
                          >
                            Abrir Link
                          </button>
                          ...
                          <button
                            ...
                            aria-label={`Copiar URL para ${service}`}
                          >
                            Copiar URL
                          </button>
                        </div>
